...
const getSalaryForMonth = async (empId, year, monthName) => {
  ...
  console.log('debug ---> Fetching salary for:', empId, year, monthName);
...
salaryHistories.forEach(s => {
  if (!salaryMap.has(s.empId)) {
    salaryMap.set(s.empId, []);
  }
  salaryMap.get(s.empId).push(s);
});

const existingSalaries = await Salary.find();
const salaryExistingMap = new Map();

existingSalaries.forEach(s => {
  const key = `${s.empId}_${s.year}_${s.month}`;
  salaryExistingMap.set(key, s);
});
...
summaries = await MonthlySummary.find().sort({ year: 1, month: 1 });

  console.log(
    'debug --> summaries for month:', summaries
  );

  console.log(`debug --> Found ${summaries.length} summaries`);
}
...
const monthNumber = Number(summary.month); // 🔥 FIX

// 🔥 NEW LOGIC (based on payroll cycle)
const salaryMonth = monthNumber;
const monthName = MONTH_MAP[Number(summary.month) - 1];
    ...
    const histories = salaryMap.get(empId) || [];

const fromDate = new Date(year, monthNumber - 1, 1);
const toDate = new Date(year, monthNumber, 0);

const effectiveSalary = histories.find(h => {
  return (
    h.effectiveFrom <= toDate &&
    (!h.effectiveTo || h.effectiveTo >= fromDate)
  );
});
    const useSalaryHistory = !!effectiveSalary;
...
const key = `${empId}_${year}_${monthName}`;
const existingSalary = salaryExistingMap.get(key);
...
const finalData = {
  ...manualFields,
  ...base   // 🔥 base should override old values
};
...
bulkOps.push({
  updateOne: {
    filter: { empId, year, month: monthName },
    update: {
      $set: {
        ...finalData,
        ...computed
  ...
  if (bulkOps.length) {
  await Salary.bulkWrite(bulkOps);
}
...
const generateSalaryFromEmployee = async (req, res) => {
  try {
    ...
    console.log('debug --> Starting salary generation from employee data...');
...
if (req.user.role === "hrms_handler" || req.user.role === "super_admin") {
  ...
  // 🔥 Step 1: get latest salary
salaries = await Salary.find().sort({ year: -1, monthNumber: -1 });
