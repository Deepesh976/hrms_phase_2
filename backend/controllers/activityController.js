...
const { generateSalaryRecords } = require('../services/activity/attendanceSalaryService');
...
const { evaluateAttendanceDay } = require('../services/attendanceLogic');
...
const uploadActivityExcel = async (req, res) => {
  try {
    ...
    const selectedUnit = req.body.unit?.trim();
...
for (const holiday of holidays) {
  await recalculateActivityForDate(holiday.date, holiday.title);
    ...
    /* ======================================================
       SANDWICH RULE SYNC FOR UPLOADED RANGE
    ====================================================== */
const dates = [];
for (let d = new Date(fromNorm); d <= toNorm; d.setDate(d.getDate() + 1)) {
  dates.push(new Date(d));
}

await Promise.all(dates.map(d => syncAttendanceForDate(d)));
...
const employeeKeys = [
  ...new Set(
    normalizedActivities.map(a => `${a.empId}__${a.empUnit}`)
  )
];

await Promise.all(
  employeeKeys.map(key => {
    const [empId, empUnit] = key.split('__');
    return regenerateMonthlySummaryForEmployee(empId, empUnit);
  })
);

// 🔥🔥🔥 ADD THIS LINE ONLY
await generateSalaryRecords();
...
await regenerateMonthlySummaryForEmployee(
  activity.empId,
  activity.empUnit
);
...
const uploadActivityData = async (req, res) => {
  try {
    ...
    const normalizedActivities = activities.map((a) => ({
      ...
      empId: String(a.empId).trim(),
      empName: String(a.empName).trim(),
      empUnit: (a.empUnit || req.body.unit)?.trim(),
    ...
    if (normalizedActivities.some(a => !a.empUnit)) {
  return res.status(400).json({
    success: false,
    message: 'Unit is required for all activities'
  });
}
...
const matchedEmployees = await Activity.find({
  ...
  ...(req.query.unit && { empUnit: req.query.unit })
}).distinct('empId');
...
const recalculateAttendance = async (req, res) => {
  try {
    ...
    const from = normalizeLocalDate(fromDate);
    const to = normalizeLocalDate(toDate);
    ...
    for (const act of activities) {
      ...
      // 🔒 DO NOT TOUCH MANUAL CHANGES
      if (
  act.isStatusModified ||
  ['ALF', 'ALH'].includes(act.status)
) continue;

      const attendance = evaluateAttendanceDay({
        inTime: act.timeInActual,
        outTime: act.timeOutActual,
        isSunday: act.date.getDay() === 0,
        isHoliday: act.status === 'HO',
        counters: { late: 0, permission: 0 }
      });

      let newStatus = 'A';

      if (attendance.ho) newStatus = 'HO';
      else if (attendance.wo) newStatus = 'WO';
      else if (attendance.present) newStatus = 'P';
      else if (attendance.half) newStatus = '½P';

      if (act.status !== newStatus) {

        // store original only once
        if (!act.originalStatus) {
          act.originalStatus = act.status;
        }
        ...
        updated++;
        employeeKeys.add(`${act.empId}__${act.empUnit}`);
      }
    }
...
await Promise.all(
  [...employeeKeys].map(key => {
    ...
    return res.json({
      success: true,
      message: `Recalculated ${updated} records`
    });
...
/* ======================================================
   🔥 GET MONTHLY SUMMARY (FIX)
====================================================== */
const getMonthlySummary = async (req, res) => {
  try {
    const { empId, empUnit, year, month } = req.query;

    if (!empId || !empUnit || !year || !month) {
      return res.status(400).json({
        success: false,
        message: 'empId, empUnit, year, month are required'
      });
    }

    const summary = await MonthlySummary.findOne({
      empId: empId.trim(),
      empUnit: empUnit.trim(),
      year: Number(year),
      month: Number(month),
    });
