...
import { FaChartBar, FaUsers, FaClipboardList, FaCheckCircle, FaBox, FaUser, FaSignOutAlt, FaBars, FaTimes } from 'react-icons/fa';
...
// HOD menu items (department-level access, can approve leaves)
const menuItems = [
  { path: '/hod-dashboard', label: 'Dashboard', icon: FaChartBar },
  { path: '/employee-info', label: 'Team Members', icon: FaUsers },
  { path: '/employee-activity', label: 'Team Attendance', icon: FaClipboardList },
  { path: '/hr-leave-requests', label: 'Leave Approvals', icon: FaCheckCircle },
  { path: '/hod-assets', label: 'Team Assets', icon: FaBox },
];
...
const HODNavbar = () => {
  ...
  const initials = (localStorage.getItem('username') || 'H').slice(0, 2).toUpperCase();
