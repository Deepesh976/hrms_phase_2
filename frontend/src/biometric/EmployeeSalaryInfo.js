...
if (monthFilter && monthFilter !== '') params.push(`month=${monthFilter}`);

if (params.length > 0) {
  url = `/salaries?${params.join('&')}`;
}

const salRes = await axios.get(url);
...
const matchMonth =
  !monthFilter ||
  String(getMonthName(item.month)) === String(monthFilter);
  ...
  const months = useMemo(() => {
    ...
    return [...new Set(data.map(d => getMonthName(d.month)))]
      .filter(Boolean)
...
const res = await axios.post('/salaries/generate-from-employee');

toast.success(res.data.message || 'Salary regenerated successfully!');
setYearFilter('');
setMonthFilter('');

// 🔥 FORCE fresh API call without filters
await axios.get('/salaries').then(res => {
  setData(Array.isArray(res.data) ? res.data : []);
      ...
      return (
        <div className="salary-container">
          ...
          <div className="salary-controls-row">

<div className="search-filter-card">

  {/* TOP ROW: Search + Download */}
  ...
  <div className="filter-row">
    ...
    {/* CARD 2 - ACTION BUTTONS (2x2 GRID) */}
    <div className="action-card-grid">

      <button className="salary-btn btn-insert" onClick={() => navigate('/input-data')}>
        <FaPlus /> Main Data
      </button>
                  ...
                  return (
                    ...
                    >
                      ...
                      {Object.keys(expectedKeys).map((k) => {
                        const rawVal = k === 'month' ? getMonthName(row[k]) : row[k];
