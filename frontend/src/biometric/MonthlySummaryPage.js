...
const getPayrollCycleFromDate = (dateStr) => {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      ...
      if (!unitParam) {
  toast.error('Unit missing in request');
  setLoading(false);
  return;
}

      console.log('🔎 Requesting:', { empId, year, month, unit: unitParam });
const res = await axios.get(
  `/activities/monthly-summary?empId=${empId}&empUnit=${unitParam}&year=${year}&month=${month}`
);
  ...
  }, [empId, dateParam, unitParam, notFoundShown]);
    ...
    <div style={styles.wrapper}>
      ...
      {!loading && summary && (
        <div style={styles.card}>
          <h3>
            {summary.empName} ({summary.empId}) - {summary.empUnit}
          </h3>
