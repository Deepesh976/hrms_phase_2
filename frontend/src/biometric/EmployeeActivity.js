...
const EmployeeActivity = () => {
  ...
  try {
    const res = await axios.get('/units'); // 🔥 your unit API
...
const handleRecalculate = async () => {
  if (!filters.startDate || !filters.endDate) {
    return toast.warn('Select start & end date to regenerate');
  }

  if (!window.confirm('Recalculate attendance for selected date range?')) return;

  setLoading(true);

  try {
    const res = await axios.post('/activities/recalculate', {
      fromDate: filters.startDate,
      toDate: filters.endDate,
    });

    toast.success(res.data.message || 'Recalculated successfully');
...
const fetchMonthlySummary = async (empId, dateStr, empUnit) => {
...
const res = await axios.get(
  `/activities/monthly-summary?empId=${empId}&empUnit=${empUnit}&year=${year}&month=${month}`
);
...
<table className="ea-table">
  <thead>
    <tr>
      ...
      <SortableHeader column="empUnit">Unit</SortableHeader>
...
onClick={() => {
  ...
  // 🔥 Prevent stale UI issue
  setTimeout(() => {
    fetchMonthlySummary(item.empId, item.date, item.empUnit);
  }, 0);
}}
    >
...
{showSummaryModal && !isRestrictedRole && (
  <div className="ea-modal-overlay">
<div
  key={summaryEmpId + summaryDate} // 🔥 ADD THIS LINE
  className="ea-modal"
